...
class BlowfishIRC():
    ###### ################
    ##
    ##  taken from: http://goo.gl/i8E8UJ
    ##  THX!
    ##  Author: Domen "iElectric" Kožar
    ##          domen@dev.si
    ##          http://www.domenkozar.com
    ##
    #### ##########
    
    def __init__(self):
        self.B64 = "./0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"
    ...
    def bytetoB64(self, s):
        r = ''
        k = -1

        while (k < len(s) - 1):
            k += 1
            left = (ord(self._i(s,k)) << 24)
            k += 1
            left += (ord(self._i(s,k)) << 16)
            k += 1
            left += (ord(self._i(s,k)) << 8)
            k += 1
            left += ord(self._i(s,k))

            k += 1
            right = (ord(self._i(s,k)) << 24)
            k += 1
            right += (ord(self._i(s,k)) << 16)
            k += 1
            right += (ord(self._i(s,k)) << 8)
            k += 1
            right += ord(self._i(s,k))

            for i in range(0,6):
                r += self.B64[right & 0x3F]
                right = right >> 6

            for i in range(0,6):
                r += self.B64[left & 0x3F]
                left = left >> 6
        return r

    def B64tobyte(self, s):
        r = ''
        k = -1
    ...
    def encrypt(self, data, key):
        ...
        for i in range(0, (8 - len(data) % 8)):
            data += chr(0)
        return self.bytetoB64(obj.encrypt(data))

    def decrypt(self, data, key):
        obj = Blowfish.new(key)
        return obj.decrypt(self.B64tobyte(data)).replace(chr(0), "")
